...
Bricks.Observer = Bricks.create({
     ...
     * @param {Function} fn Функция, вызываемая при возникновении события.
     */
    ...
    _fireEvent: function(name, data) {
        var listenerSets = Bricks.getPrototypeChain(this, '$$_listeners');
        for (var j = 0; j < listenerSets.length; j++) {
            var listeners = listenerSets[j].$$_listeners[name] || [];
            if (listenerSets[j].$$_listeners['*']) {
                listeners = listeners.concat(listenerSets[j].$$_listeners['*']);
            }
