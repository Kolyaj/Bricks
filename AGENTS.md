...
- Тесты: `npm test` — Node-харнесс `test/node/run.js` (ADR 0003, основной): компилирует `test/tests/*.js` in-process через API dresscodejs (эквивалент `-d`), оборачивает бандл CJS-фабрикой со стандартной инъекцией (`BricksTest`, `window`, `document`, `location`, `ActiveXObject`, `setTimeout`/`clearTimeout` = fake-часы, `describe`, `it`, `assert`); весь сьюит — два прохода, sloppy + strict (`'use strict'` в теле фабрики); любое падение роняет exit-code — known-red-механизма нет, баги в lib исправляются вместе с созданием тестов (решение 2026-09); fake-часы на `BricksTest.clock`; браузерные профили — plain JS в `test/profile/` (в Node `require`, в браузере один `<script>`). Браузерная часть — Yaxy + `test/index.html` (mocha/sinon/jQuery из `test/external/`), остаточный минимум.
