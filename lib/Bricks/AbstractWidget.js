...
//#include DOM.js::getEl::normalizeCSSProperty::createFragment
...
Bricks.AbstractWidget = Bricks.Component.inherit({
    ...
    _buildCss: function() {
        ...
        if (cssText) {
            var stylesId = 'Bricks_AbstractWidget__styles';
    ...
    _applyTemplate: function(tpl) {
        if (tpl) {
            if (!this.$__tplsCache[tpl]) {
                this.$__tplsCache[tpl] = Bricks.String.compile(tpl);
            }
            return this.$__tplsCache[tpl].call(this);
        } else {
            return '';
        }
    }
