...
/**
 * Возвращает true, если объект является массивом, иначе false.
 *
 * @param {*} obj Проверяемый объект.
 *
 * @return {Boolean}
 *
 * @link Bricks.Array.isArray
 * @deprecated
 */
 ...
 * @param {Object} ...source Объекты-источники: собственные свойства каждого копируются в dst.
 *
 ...
 * Функция обходит цепочку по свойству superclass, поэтому объект должен принадлежать классу, созданному с помощью {@link Bricks.create}
 *
 * @param {Object} object Конструктор или объект, принадлежащий такому классу.
 * @param {String} [prop] Свойство, наличие которого требуется для включения объекта в цепочку.
 *
 * @return {Object[]} Массив объектов цепочки.
 */
...
/**
 * Возвращает значения свойства prop из цепочки прототипов от object до корневого прототипа.
 * Функция обходит цепочку по свойству superclass, поэтому объект должен принадлежать классу, созданному с помощью {@link Bricks.create}
 *
 * @param {Object} object Конструктор или объект, принадлежащий такому классу.
 * @param {String} prop Имя свойства.
 *
 * @returns {Array} Значения свойства из объектов цепочки.
 */
Bricks.getPrototypeChainValues = function(object, prop) {
    return Bricks.getPrototypeChain(object, prop).map(function(item) {
        return item[prop];
    });
};
 ...
 *
 * @return {Number} Случайное целое число в диапазоне [start, end].
 */
