 ...
 *
 * @return {Function} Функция, принимающая DOM-элемент и возвращающая true, если элемент соответствует селектору.
 */
 ...
 * @param {String} name Имя свойства или несколько через запятую.
 * @param {*} value Значение свойства или массив значений, если свойств несколько.
 */
Bricks.DOM.setStyleProperty = function(el, name, value) {
    ...
    for (var i = 0; i < names.length; i++) {
        if (names[i].indexOf('--') === 0) {
            el.style.setProperty(names[i], Bricks.DOM.normalizeCSSValue(value[i]));
        } else {
            var propValue = Bricks.DOM.normalizeCSSProperty(names[i], value[i] === null || value[i] === undefined ? '' : value[i], el);
            el.style[propValue[0]] = Bricks.DOM.normalizeCSSValue(propValue[1]);
        }
    }
};

/**
 * Приводит значение CSS-свойства к строке: числу дописывается px, null и undefined превращаются в пустую строку.
 *
 * @param {Number|String} value Значение свойства.
 *
 * @return {String} Готовое к присваиванию значение.
 */
Bricks.DOM.normalizeCSSValue = function(value) {
    return typeof value === 'number' ? value + 'px' : value === null || value === undefined ? '' : value;
};
 ...
 * @param {Node/String} el Элемент или его id.
 * @param {String} className Новое значение className.
 */
...
Bricks.DOM.toggleClassName = function(el, className, adding) {
    var was = Bricks.DOM.classNameExists(el, className);
    if (arguments.length < 3) {
        adding = !was;
    }
    if (adding) {
        Bricks.DOM.addClassName(el, className);
    } else {
        Bricks.DOM.removeClassName(el, className);
    }
    return was !== adding;
};

/**
 * Возвращает родителя элемента. Если указан selector, то производит поиск вверх по цепочке родителей, пока
...
/**
 * Проверяет, является ли child потомком parent (или самим parent).
 *
 * @param {Node/String} parent Элемент или его id.
 * @param {Node/String} child Элемент или его id.
 * @returns {boolean} true, если child является потомком parent или совпадает с ним.
 */
...
/**
 * Снимает с элемента el обработчик fn с событий events, перечисленных через запятую. Обработчик должен быть
...
/**
 * Снимает все обработчики, навешенные методом {@link Bricks.DOM.on}.
 */
...
Bricks.DOM.getViewportSize = function(doc) {
    ...
    var win = Bricks.DOM.getWindow(doc);
    return [Math.max(win.innerWidth || 0, root.clientWidth), Math.max(win.innerHeight || 0, root.clientHeight)];
};
 ...
 *
 * @param {String} str HTML-строка, из которой создаётся фрагмент.
