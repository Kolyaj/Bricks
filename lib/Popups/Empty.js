...
Popups.Empty = Bricks.Widget.inherit({
    ...
    _initComponent: function() {
        ...
        if (!this._renderTo) {
            this._doc.body.insertBefore(this._getEl(), this._doc.body.firstChild);
        }
    ...
    destroy: function() {
        this._un(this._doc, 'click', this.Popups_Empty__onDocumentClick);
    ...
    showBy: function(el, orientations) {
        ...
        this._visibledBy = el;
        this.showByRect(Bricks.DOM.getPos(el), Bricks.DOM.getSize(el), orientations);
    },

    /**
     * Показывает попап с носиком рядом с прямоугольником с координатами rectPos и с размерами rectSize.
     *
     * @param {Array} rectPos Координаты прямоугольника.
     * @param {Array} rectSize Размеры прямоугольника.
     * @param {Array/String} [orientations] Строка 'top', 'right', 'bottom', 'left' или массив таких строк. Указывает
     *      с какой стороны от элемента показывать попап. Если передан массив, то выбирается первое оптимальное
     *      значение. По умолчанию: ['right', 'left', 'bottom', 'top'].
     */
    showByRect: function(rectPos, rectSize, orientations) {
        orientations = orientations || ['right', 'left', 'bottom', 'top'];
        if (!Bricks.isArray(orientations)) {
            orientations = [orientations];
        }

        var tailClassNames = ['popups-empty_tail-top', 'popups-empty_tail-right', 'popups-empty_tail-bottom', 'popups-empty_tail-left'];
        for (var c = 0; c < tailClassNames.length; c++) {
            this._removeClassName(tailClassNames[c]);
        }
        ...
        var rectCenter = [
            Math.round(rectPos[0] + rectSize[0] / 2),
            Math.round(rectPos[1] + rectSize[1] / 2)
        ];
        var scrollPos = Bricks.DOM.getDocumentScroll(this._doc);
        var viewportSize = Bricks.DOM.getViewportSize(this._doc);
        var documentSize = Bricks.DOM.getDocumentSize(this._doc);
        ...
        for (var i = 0; i < orientations.length; i++) {
            ...
            }[orientations[i]].call(this, rectPos, rectSize, rectCenter, scrollPos, viewportSize, documentSize, popupSize);
        ...
        })[bestOrientation].call(this, rectPos, rectSize, rectCenter, scrollPos, viewportSize, documentSize, popupSize);
    },
    ...
    showAbout: function(point) {
        ...
        var viewport = Bricks.DOM.getViewportSize(this._doc);
        var scroll = Bricks.DOM.getDocumentScroll(this._doc);
