...
**Тестовый харнесс**:
Среда запуска тестов. Два харнесса: Node (vm-контекст с pre-declared `Bricks`/`window`/`document`, `npm test` — основной) и браузер (Yaxy + `test/index.html` — остаточный минимум: то, что нельзя mock-ать).
